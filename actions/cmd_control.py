...
WIN_COMMAND_MAP = [
    ...
    (["large files", "biggest files", "largest files"],
     f'powershell "Get-ChildItem \\"{Path.home().anchor}\\\" -Recurse -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object -First 10 FullName,Length | Format-Table"', False),
]
...
def cmd_control(
    ...
    if not command:
        ...
        if command:
            ...
            print(f"[CMD] 🤖 Ollama fallback for: {task}")
            command = _ask_ollama(task)
