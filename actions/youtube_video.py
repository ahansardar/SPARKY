...
def _resolve_tool(exe_name: str) -> str | None:
    ...
    if resolved:
        return resolved
    candidate = _LOCAL_FFMPEG_BIN / f"{exe_name}.exe"
    if candidate.exists():
...
def _is_audio_playing() -> bool:
    with _audio_lock:
        return _audio_process is not None and _audio_process.poll() is None


def _set_audio_paused(paused: bool) -> bool:
    global _audio_paused
    with _audio_lock:
        ...
        try:
            import psutil
            proc = psutil.Process(_audio_process.pid)
            if paused:
                proc.suspend()
                _audio_paused = True
            else:
...
def youtube_video(
    ...
    Actions:
        ...
        pause     : Pause direct song playback
        resume    : Resume direct song playback
        stop      : Stop direct song playback
