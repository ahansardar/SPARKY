...
# SPARKY — Computer Settings & UI Controls
#
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            endpoint.SetMasterVolumeLevelScalar(float(value) / 100.0, None)
            endpoint.SetMute(1 if value == 0 else 0, None)
            print(f"[Settings] Volume -> {value}% (pycaw)")
            return
        except Exception as e:
            print(f"[Settings] pycaw failed: {e}")
            try:
                # Coarse but reliable fallback if pycaw API varies by version.
                pyautogui.press("volumedown", presses=60, interval=0.01)
                steps_up = int(round(value / 2.0))
                if steps_up > 0:
                    pyautogui.press("volumeup", presses=steps_up, interval=0.01)
                print(f"[Settings] Volume -> {value}% (keyboard fallback)")
                return
            except Exception as kerr:
                print(f"[Settings] keyboard volume fallback failed: {kerr}")
    elif _OS == "Darwin":
...
def computer_settings(
    ...
    if action == "volume_set":
        try:
            target = 50 if value in (None, "") else int(value)
            volume_set(target)
