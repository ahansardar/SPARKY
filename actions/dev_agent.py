...
PROJECTS_DIR       = Path.home() / "Desktop" / "SparkyProjects"
MAX_FIX_ATTEMPTS   = 4
MODEL_PLANNER      = "llama3:8b"
MODEL_WRITER       = "llama3:8b"


def _get_model(model_name: str):
    return OllamaTextModel(model_name)
...
def _open_vscode(project_dir: Path) -> bool:
    vscode_paths = ["code"]
    env_cli = os.getenv("VSCODE_CLI", "").strip()
    if env_cli:
        vscode_paths.append(env_cli)
...
def _build_project(
    ...
    proj_name = project_name or plan.get("project_name", "sparky_project")
