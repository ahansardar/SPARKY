...
#   User request → Ollama generates Python/pyautogui code → Safety check → Execute
#
...
def _ask_ollama_for_desktop_action(task: str) -> str:
    """
    Asks Ollama to generate safe Python/pyautogui code
...
def _execute_generated_code(code: str) -> str:
    """Safely executes LLM-generated desktop automation code."""
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            print(f"[Desktop] 🤖 Asking Ollama: {actual_task}")
            if player:
                player.write_log("[Desktop] Generating action...")

            code = _ask_ollama_for_desktop_action(actual_task)
            ...
            if full_task:
                code   = _ask_ollama_for_desktop_action(full_task)
